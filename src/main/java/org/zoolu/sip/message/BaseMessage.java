...
/**
 * Class BaseMessage implements a generic SIP Message. Only RFC3261 parts,
 * others are implemented in Message.java
 */
public abstract class BaseMessage extends org.zoolu.tools.MonitoredObject {
	...
	public boolean isRequest() throws NullPointerException { // Req-Line = Method ' ' SIP-URL ' ' "SIP/2.0" CRLF
		...
		if (proto_version == null || proto_version.length() < SIP_VERSION.length())
			return false;
		proto_version = proto_version.substring(0, SIP_VERSION.length());
		if (proto_version.equalsIgnoreCase(SIP_VERSION))
			return true;
		return false;
	}
	...
	/**
	 * Gets the RequestLine of the Message (returns null if called for no request
	 * message).
	 */
	...
	public boolean isResponse() throws NullPointerException { // Status-Line = "SIP/2.0" ' ' Status-Code '
																// 'Reason-Phrase" CRLF
																// if (message==null) return false;
		if (message == null || message.length() < SIP_VERSION.length())
			return false;
	...
	/**
	 * Gets the StautsLine of the Message (returns null if called for no response
	 * message).
	 */
	...
	/**
	 * Adds a one or more Headers at the top/bottom. The bottom is considered before
	...
	public RecordRouteHeader getRecordRouteHeader() { // Header h=getHeader(SipHeaders.Record_Route);
														// if (h==null) return null; else return new
														// RecordRouteHeader(h);
	...
	/**
	 * Checks whether the next Route is formed according to RFC2543 Strict Route and
	 * adapts the message.
	 */
	public void rfc2543RouteAdapt() {
		if (hasRouteHeader()) {
			...
			if (!(new RouteHeader(mrh.getTop())).getNameAddress().getAddress().hasLr()) { // re-format the message
																							// according to the RFC2543
																							// Strict Route rule
