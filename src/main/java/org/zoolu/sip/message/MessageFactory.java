...
public class MessageFactory extends org.zoolu.sip.message.BaseMessageFactory {
	...
	public static Message createMessageRequest(SipProvider sip_provider, NameAddress recipient, NameAddress from,
		...
		Message req = createRequest(sip_provider, SipMethods.MESSAGE, request_uri, recipient, from, null, callid, cseq,
				localtag, null, null, null);
		if (subject != null)
	...
	public static Message createReferRequest(SipProvider sip_provider, NameAddress recipient, NameAddress from,
			NameAddress contact, NameAddress refer_to/* , NameAddress referred_by */) {
		SipURL request_uri = recipient.getAddress();
		String callid = sip_provider.pickCallId();
		int cseq = SipProvider.pickInitialCSeq();
		String localtag = SipProvider.pickTag();
		// String branch=SipStack.pickBranch();
		Message req = createRequest(sip_provider, SipMethods.REFER, request_uri, recipient, from, contact, callid, cseq,
				localtag, null, null, null);
	 ...
	 * <p>
	 * parameters: <br>
	 * - <i>refer_to</i> mandatory <br>
	 * - <i>referred_by</i> optional
	 */
	 ...
	 */
	public static Message createSubscribeRequest(SipProvider sip_provider, SipURL recipient, NameAddress to,
			NameAddress from, NameAddress contact, String event, String id, String content_type, String body) {
	...
	public static Message createSubscribeRequest(Dialog dialog, String event, String id, String content_type,
			String body) {
	...
	public static Message createNotifyRequest(Dialog dialog, String event, String id, String content_type,
			String body) {
	...
	public static Message createPrack(Dialog dialog, Message msg) {		
		...
		CSeqHeader csh = msg.getCSeqHeader();
		long rseq = msg.getRSeqHeader().getSequenceNumber();
		RAckHeader rh = new RAckHeader(rseq,csh.getSequenceNumber(),csh.getMethod());
		req.setHeader(rh);
		req.setHeader(msg.getRequireHeader());
		req.setHeader(msg.getSupportedHeader());
		req.setToHeader(msg.getToHeader());
		return req;
	}
